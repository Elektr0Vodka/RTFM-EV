"""Tests for database migration 102: add packet_group_by_content column."""
...
class TestMigration102:
    """Test migration 102: add the shared 'group repeats by content' toggle column."""
    ...
    async def test_adds_packet_group_by_content_column(self):
        ...
        try:
            ...
            await conn.commit()
            # Start just below 102 so only this migration runs.
            await set_version(conn, 101)

            applied = await run_migrations(conn)

            assert applied == LATEST_SCHEMA_VERSION - 101
