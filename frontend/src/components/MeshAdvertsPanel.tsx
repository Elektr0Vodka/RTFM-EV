/**
 * MeshAdvertsPanel.tsx
 *
 * The "Adverts" view of the Mesh Health page: advert-frequency alerts for
 * contacts advertising too often, signal/hop distributions, SNR-RSSI scatter,
 * hourly heatmap, relay pairs, and a sortable, paginated contacts table.
 *
 * Rendered inside the MeshHealthView shell, which owns the page header, the
 * Adverts/Requests pill, the shared time-window selector, and the refresh
 * control. This panel returns only its content blocks.
 */
...
export function MeshAdvertsPanel({
  config,
  selectedWindow,
  refreshKey,
  onNavigateToMap,
  ...
  // Initial fetch, window-change fetch, and manual-refresh fetch (refreshKey)
  useEffect(() => {
    fetchHealth(selectedWindow);
    setPage(0);
  }, [selectedWindow, refreshKey, fetchHealth]);

  // Fetch extra analytics charts on window change / refresh
  useEffect(() => {
    const endTs = Math.floor(Date.now() / 1000);
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-border bg-background">
                  ...
                  <th className={`${thClass} text-right`} onClick={() => handleSort('last_seen')}>
