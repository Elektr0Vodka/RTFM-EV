...
describe('usePacketHistory', () => {
  ...
  it('fetches on mount, stores oldest-first, exposes nextCursor', async () => {
    ...
    const { result } = renderHook(() =>
      usePacketHistory({ startTs: 0, endTs: 300, filters: allEnabled })
    );
  ...
  it('does not stream live packets: rows come only from the query', async () => {
  ...
  it('refetches when refreshToken changes (manual refresh)', async () => {
    getPacketHistory
      .mockResolvedValueOnce({ packets: [pkt(9, 200)], next_cursor: null })
      .mockResolvedValueOnce({ packets: [pkt(9, 200), pkt(10, 250)], next_cursor: null });
    const { result, rerender } = renderHook(
      ({ token }) =>
        usePacketHistory({ startTs: 0, endTs: 300, filters: allEnabled, refreshToken: token }),
    ...
    rerender({ token: 1 });
    await waitFor(() => expect(result.current.rows.length).toBe(2));
    expect(getPacketHistory).toHaveBeenCalledTimes(2);
  });
});
