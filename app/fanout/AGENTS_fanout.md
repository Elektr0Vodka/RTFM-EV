...
- Per-topic toggles + cadence (all community presets, incl. the DMC/analyzer broker presets):
  ...
  - `publish_config` (bool, default `false`) - opt-in retained `meshcore/{IATA}/{PUBKEY}/config` topic, RTFM-EV's counterpart of the DMC observer firmware `config` topic (type 5, `MQTTMessageBuilder::buildConfigMessage`). Built by `_format_node_config` (pure, tested) and published by `CommunityMqttPublisher._publish_config` right after `status` on connect and on the status cadence. Sections: identity (`origin`, `origin_id`, `node_name`, `model`, `firmware_version`, `uptime_secs`, `client_version`), `radio` (freq/bw/sf/cr/tx_power/max_tx_power/multi_acks from `self_info`), `repeat` (`disable_fwd` = host repeater not armed, `flood_max*`, `loop_detect`), `region_gate` (host repeater `dc_gate_*`), `region` (`home`, `default` = app `flood_scope`, `wildcard_flood` = `unscoped_flood_allow`, `scopes[]` = host repeater region list as `{name, flood, parent}` with `"*"` for top level), `host_repeater.state` (RTFM-EV extra) and `mqtt` (the per-topic toggles, `status_interval`, `iata`). Firmware-only sections (bridge, gps, power, room, timezone, alert, snmp) are omitted. Never includes broker host, credentials or keys.
