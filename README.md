...
* Channel image chunks sent by meshcore-open (`GRP_DATA` packets) are decrypted with the channel key and shown as an "Image (not supported)" placeholder in the channel, one per image. Images are not decoded (that needs meshcore-open's neural codec) and the image bytes are not stored, only the chunk metadata Fanout integrations (bots, MQTT, webhooks, Apprise, SQS) do not receive these placeholder rows unless the integration's scope opts in with "Forward channel data placeholders"; bots never do.
...
- Device history: each position a contact has advertised, listed under
...
**Policy rules.** The rule list (OpenHop format, first match wins) can match on the decrypted channel name, the flood region (`unscoped` for plain floods), the first or last relay in the path or the whole path as a `10>a1>b2` string, and the `matches` operator takes a regular expression (anchor with `^`/`$`, `(?i)` for case-insensitive) for sender or message text. A rule can also decide only a percentage of its matches (**match probability**, deterministic per packet) or let one match per window through and act on the rest (**throttle**, budget per rule, per sender, per channel or per first hop); a rule that steps aside lets the next rule decide, so a probe rule with `log_only` above a drop rule counts what the drop would catch. Each rule shows its hits, throttle passes and the estimated airtime its drops saved; the stats pane totals the saved airtime of all rule and rate-limiter drops. Modelled on the [jhuebert/MeshCore](https://github.com/jhuebert/MeshCore) repeater packet filter.
